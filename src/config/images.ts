...
export function handleImageError(
  ...
  const variants =
    ...
    (image?.basename ? [image.basename] : null) ||
    (el.dataset.imgBase ? [el.dataset.imgBase] : []);

  const allVariants =
    image && "basenameVariants" in image && image.basenameVariants?.length
      ? image.basenameVariants
      : variants;

  if (folder && allVariants.length) {
    const variantIndex = Number(el.dataset.variantIndex ?? "0");
    const extIndex = Number(el.dataset.extIndex ?? "0");

    if (extIndex + 1 < IMAGE_EXTENSIONS.length) {
      el.dataset.extIndex = String(extIndex + 1);
      el.src = localImagePath(folder, allVariants[variantIndex], extIndex + 1);
      return;
    }

    if (variantIndex + 1 < allVariants.length) {
      el.dataset.variantIndex = String(variantIndex + 1);
      el.dataset.extIndex = "0";
      el.src = localImagePath(folder, allVariants[variantIndex + 1], 0);
      return;
    }
  }
...
const cutBasenames = (n: number) => [`g (${n})`, `g(${n})`];
