...
> 📌 **The easiest GitHub Pages setup is already configured for you.** Running
> `npm run build` automatically copies the built site into the `docs/` folder, so
> you can just push to `main` and enable "Deploy from a branch → main → /docs"
> in GitHub Pages settings. See the quick guide in the [README](../README.md).
